...
// Mock exceptions data with detailed information
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (status && status !== "all") {
      filteredExceptions = filteredExceptions.filter(exc => exc.status === status);
    }

    if (severity && severity !== "all") {
      filteredExceptions = filteredExceptions.filter(exc => exc.severity === severity);
    }
